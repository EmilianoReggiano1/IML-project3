...
class AdaBoost(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        for _ in range(self.iterations_):
            ...
            model = self.wl_()
            weighed_labels = D * y
            model.fit(X, weighed_labels)
            # Predict responses
            y_pred = model.predict(X)
            # Calculate weighted error
            epsilon = np.sum(D * (y != y_pred))
            model_weight = 0.5 * np.log((1 - epsilon) / epsilon)
            # Update distribution weights
            D = D * np.exp(-model_weight * y * y_pred)
            D = D / np.sum(D)
            # Add model and weight to lists
            models.append(model)
    ...
    def partial_predict(self, X: np.ndarray, T: int) -> np.ndarray:
        ...
        weighted_predictions = np.array(
            [model.predict(X) * weight for model, weight in zip(self.models_[:T], self.weights_[:T])])
        total_predictions = np.sign(np.sum(weighted_predictions, axis=0))
